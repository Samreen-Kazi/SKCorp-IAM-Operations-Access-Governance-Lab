...
**Hybrid Identity and Access Management Lab | Active Directory, Windows Server 2022, PowerShell**

- Built an enterprise-style Active Directory environment with Windows Server 2022, DNS, and a domain-joined Windows 11 endpoint.
- Implemented Joiner-Mover-Leaver workflows by provisioning, modifying, and deprovisioning user accounts and access rights.
- Designed RBAC using Active Directory security groups and enforced departmental access through NTFS and SMB permissions.
- Configured delegated administration for Helpdesk password reset workflows using least-privilege principles.
- Automated user access reviews with PowerShell by generating CSV reports of user group memberships and entitlements.

## Future Improvements

- Integrate with Microsoft Entra ID using a personal/admin tenant.
- Add Conditional Access and MFA policies.
- Expand PowerShell automation for bulk user provisioning.
- Add audit logging and event log monitoring.
